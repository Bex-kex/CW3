...
import copy
import time
from SudokuBoard import SudokuBoard, SudokuNumber
...
def find_empty(grid: List[List[SudokuNumber]]):
	'''
...
def recursive_solve(board: SudokuBoard):
	'''
	...
	emptycell = find_empty(board.object_list)
	
	#If there's no empty places left, check if we've found a solution
	if not find_empty(board.object_list):
		#If the solution is correct, return it.
		if check_solution(board, board.dims_y, board.dims_x):
			return board.object_list 
		else:
	...
	for i in emptycell.available_values:
			
			#Place the value into the grid
			emptycell.current_value = i
			#Recursively solve the grid
			
			
			ans = recursive_solve(board)
...
def solve(grid, n_rows, n_cols):
	...
	board = SudokuBoard(grid,n_rows,n_cols)
	return recursive_solve(board)
